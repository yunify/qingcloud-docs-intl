...
function get_select_content(i,m,url_path){
	$.getJSON("/category.json", function (data){
		...
		for(var k in child_data['user_doc']){
			...
			select_content += '<option value="'+child_data['user_doc'][k]['doc_url']+'" '+selected+'>User :'+child_data['user_doc'][k]['example_title']+'</option>';
		}
		...
		for(var k in child_data['development_doc']){
			...
			select_content += '<option value="'+child_data['development_doc'][k]['doc_url']+'" '+selected+'>Developer:'+child_data['development_doc'][k]['example_title']+'</option>';
		}
		$('.select-field').find('select').append(select_content);
	})
}